<x-admin-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('admin.role.add') }}" class="bg-slate-700 hover:bg-slate-800 text-white font-bold py-2 px-4 rounded">Add Role</a>
        </div>
    </x-slot>
    @if(session('success'))
                        ...
                        @foreach ($roles as $role)
                            <tr>
                                ...
                                <td class="flex justify-center">
                                    <a href="{{ route('admin.role.show', $role->id) }}" class="mr-3 bg-yellow-500 text-white py-2 px-4 rounded-md">Edit</a>
                                    <form action="{{ route('admin.role.delete', $role->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this?');">
                                        @csrf
                                        @method('DELETE')
